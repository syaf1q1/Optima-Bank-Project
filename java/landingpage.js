...
// ============================================
// VIDEO ADS FUNCTIONALITY WITH SKIP BUTTON
// ============================================
...
function resetSkipButton() {
  ...
  skipAdBtn.style.pointerEvents = 'none';
  skipAdBtn.innerHTML = `Skip in <span id="skipTimer">${skipCountdown}</span>s`;
...
function startSkipCountdown() {
  skipInterval = setInterval(() => {
    ...
    if (timerSpan) {
      timerSpan.textContent = skipCountdown;
    }
    
    if (skipCountdown <= 0) {
      clearInterval(skipInterval);
      skipAdBtn.classList.add('active');
      skipAdBtn.innerHTML = 'Skip Ad ›';
    }
  }, 1000);
}
...
if (skipAdBtn) {
  skipAdBtn.addEventListener("click", () => {
    if (skipAdBtn.classList.contains('active')) {
      adsModal.style.display = "none";
      adsVideo.pause();
      resetSkipButton();
      
      if (!adWatchedFully) {
        addNotification("Ad skipped. Watch full ads to earn points!", 'info');
      }
    }
  });
}

// Close Ads Modal (X button)
if (closeAdsModal) {
...
if (adsVideo) {
  ...
  adsVideo.addEventListener("pause", () => {
    if (skipInterval) {
      clearInterval(skipInterval);
    }
  });
  
  // Video Play - Resume Countdown
  adsVideo.addEventListener("play", () => {
    if (skipCountdown > 0 && !skipInterval) {
      startSkipCountdown();
    }
  });
}
